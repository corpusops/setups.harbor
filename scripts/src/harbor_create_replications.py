...
def main(**cli):
    ...
    for ns, nsdata in sorted(hdata['projects'].items()):
        ...
        for img, idata in nsdata['repos'].items():
            ...
            # (current interface limit of harbor)
...
def add_replication_rule(registry, ns, img, tags,
    ...
    try:
        ...
        ret = harbor_api('/replication/policies', method='post', json=rdata, expect=201)

        repl = harbor_api(ret.headers['Location'], force_uri=True).json()
    ...
    for i, v in repl_data.items():
        ...
        if repl.get(i) != v:
            ...
            ret = harbor_api(f'/replication/policies/{repl["id"]}', method='put', json=repl, expect=200)
            repl = harbor_api(f'/replication/policies/{repl["id"]}').json()
            replications[repl['name']] = repl
            break
    if repl.get('enabled'):
        if not filter_execs(repl, status=['succeed', 'inprogress']):
            L.info(f"Starting replication {repl['name']}")
            ret = harbor_api("/replication/executions", method='post',
