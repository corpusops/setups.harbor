...
default_dry_run = os.environ.get('HARBOR_INVITE_DRYRUN',
...
HARBOR_DATAFILE = os.environ.get("HARBORR_DATAFILE_JSON", "data/harbor.json")
...
def ensure_harbor_connected():
    user = harbor_api('/users/current', expect=200)
    return user.json()
...
def create_robot(user, prefix='bot__', projects=None, **kw):
    ...
    bot.update(**kw)
    ret = harbor_api('/robots', method='post', json=bot, expect=201)
...
def get_harbor_data(harbordatafile=HARBOR_DATAFILE):
    try:
        d = _vars['harbordata']
        assert d
    except (KeyError, AssertionError):
        pass
    if not os.path.exists(harbordatafile):
        write_harbor_data({}, harbordatafile)
    with open(harbordatafile) as fic:
        d = _vars['harbordata'] = json.load(fic)
    return d


def write_harbor_data(data=None, harbordatafile=HARBOR_DATAFILE):
    if data is None:
        data = get_harbor_data(harbordatafile=harbordatafile)
    with open(harbordatafile, "w") as fic:
        json.dump(data, fic, indent=2, sort_keys=True)


def gen_pw(password_length=None):
    """Default password will meet this requirement: harbor passwords needs at least one uppercase, downcase and 1 digit"""
...
def configure_harbor_user(
    user_or_username,
    email=None,
    password=None,
    harbordatafile=HARBOR_DATAFILE,
    realname=None,
    sysadmin=None,
    hdata=None,
    ...
    **kw
    ...
    if hdata is None:
        hdata = get_harbor_data(harbordatafile=harbordatafile)
    husers = hdata["users"]
...
def get_or_patch_user(
    ...
    **kw,
    ...
    udata = configure_harbor_user(
        user_or_username, email=email, realname=realname,
        password=password, harbordatafile=harbordatafile,
    ...
    users = dict([(a['username'], a) for a in get_harbor_batched('/users')])
    try:
        user = users[username]
        ...
        ret = harbor_api(
            ...
            json={"username": username,
                  ...
                  "password": password},
            expect=201)
        user = harbor_api(ret.headers['Location'], force_uri=True).json()
...
def notify_access(
    ...
    assert mail_login
    assert mail_pw
    subject = f"Your harbor {HARBOR_URL} access"
    ...
    if dry_run:
        ...
        s.quit()
        configure_harbor_user(user, notified=True)
    return msg
...
def stop_execs(repl_data=None):
    ...
    for exe in harbor_api(
        ...
        if exe['in_progress']:
            ...
            while True:
                exe = harbor_api(f'/replication/executions/{exe["id"]}').json()
                if exe['in_progress']:
